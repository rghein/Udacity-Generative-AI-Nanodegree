...
class ChromaEmbeddingPipelineTextOnly:
    ...
    def __init__(self, 
        ...
        self.embedding_function = OpenAIEmbeddingFunction(api_key=self.openai_api_key, model_name=self.embedding_model)
    ...
    def chunk_text(self, text: str, metadata: Dict[str, Any]) -> List[Tuple[str, Dict[str, Any]]]:
        ...
        if len(text) <= self.chunk_size:
            ...
            chunks.append((text, chunk_metadata))
            return chunks
        ...
        while start_index < len(text):
            ...
            if end_index < len(text):
                search_range_start = max(start_index, end_index - self.chunk_overlap)
                possible_split_point = -1
                for i in range(end_index - 1, search_range_start - 1, -1):
                    if text[i] in ['.', '!', '?'] and (i + 1 == len(text) or text[i + 1].isspace()):
                        possible_split_point = i + 1
                        break
                if possible_split_point != -1 and possible_split_point > start_index:
                    chunk_content = text[start_index:possible_split_point]
                    end_index = possible_split_point

            # TODO: Create metadata for each chunk
            chunk_metadata = metadata.copy()
            chunk_metadata['chunk_index'] = chunk_index
            chunk_metadata['start_char'] = start_index
            chunk_metadata['end_char'] = end_index

            chunks.append((chunk_content, chunk_metadata))

            # TODO: Implement chunking logic with overlap
            new_start = end_index - self.chunk_overlap
            if new_start <= start_index:
                new_start = start_index + self.chunk_size
    ...
    def generate_document_id(self, file_path: Path, metadata: Dict[str, Any]) -> str:
        ...
        formatted_chunk_index = f"{chunk_index:04d}"

        doc_id = f"{mission}_{source}_chunk_{formatted_chunk_index}"
        return doc_id
    ...
    def add_documents_to_collection(self, documents: List[Tuple[str, Dict[str, Any]]], 
        ...
        # replace mode: delete all existing chunks from this file first
        if update_mode == "replace":
            existing_ids = self.get_file_documents(file_path)
            if existing_ids:
                self.collection.delete(ids=existing_ids)
                logger.info(f"Deleted {len(existing_ids)} existing chunks for {file_path.name}")
    ...
    def process_all_text_data(self, base_path: str, update_mode: str = 'skip') -> Dict[str, int]:
        ...
        for file_path in files:
            try:
                ...
                # update stats
                stats['documents_added'] += results['added']
                stats['documents_updated'] += results['updated']
                stats['documents_skipped'] += results['skipped']
                stats['files_processed'] += 1
                stats['missions'][mission]['files'] += 1
                stats['missions'][mission]['chunks'] += chunk_count
                stats['missions'][mission]['added'] += results['added']
                stats['missions'][mission]['updated'] += results['updated']
                stats['missions'][mission]['skipped'] += results['skipped']
